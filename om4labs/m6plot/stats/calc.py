...
def calc(s, area, s2=None, debug=False):
  ...
  weight = area.copy()
  if debug: print('stats: sum(area) =',numpy.ma.sum(weight))
  if not numpy.ma.getmask(s).any()==numpy.ma.nomask: weight[s.mask] = 0.
  sumArea = numpy.ma.sum(weight)
  if debug: print('stats: sum(area) =',sumArea,'after masking')
  if debug: print('stats: sum(s) =',numpy.ma.sum(s))
  if debug: print('stats: sum(area*s) =',numpy.ma.sum(weight*s))
  ...
  if debug: print('stats: mean(s) =',mean)
  if debug: print('stats: std(s) =',std)
  if debug: print('stats: rms(s) =',rms)
