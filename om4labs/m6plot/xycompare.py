...
def xycompare(field1, field2, x=None, y=None, area=None,
  ...
  xlabel, xunits, ylabel, yunits = formatting.createXYlabels(x, y, xlabel, xunits, ylabel, yunits)
  ...
  dMin, dMax, dMean, dStd, dRMS = stats.calc(maskedField1 - maskedField2, area, debug=debug)
  if s1Mean is not None: dRxy = stats.corr(maskedField1 - s1Mean, maskedField2 - s2Mean, area)
  else: dRxy = None
  s12Min = min(s1Min, s2Min); s12Max = max(s1Max, s2Max)
  xLims = formatting.boundaryStats(xCoord); yLims = formatting.boundaryStats(yCoord)
  if debug:
  ...
  cmap, norm, extend = cm.chooseColorLevels(s12Min, s12Max, colormap, clim=clim, nbins=cBins, extend=extend)
  ...
  if npanels in [1,3]:
    ...
    if sector == None or sector == 'global':
      ...
      if dlim is None and dStd>0:
        cmap, norm, dextend = cm.chooseColorLevels(dMean-sigma*dStd, dMean+sigma*dStd, dcolormap, clim=dlim, nbins=nbins, \
                                                extend='both', autocenter=True)
      else:
        cmap, norm, dextend = cm.chooseColorLevels(dMin, dMax, dcolormap, clim=dlim, nbins=nbins, extend=dextend, autocenter=True)
      ...
      dMin, dMax, dMean, dStd, dRMS = stats.calc(maskedDiffField, areaCopy, debug=debug)
