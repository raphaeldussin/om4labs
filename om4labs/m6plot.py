...
try:
    ...
    print("Basemap module not found. Some regional plots may not function properly")
...
def xyplot(
  ...
  """
    ...
    if len(xlabel + xunits) > 0:
        plt.xlabel(label(xlabel, xunits))
...
def xycompare(
  ...
  """
    ...
    dMin, dMax, dMean, dStd, dRMS = myStats(
    ...
    if npanels in [1, 3]:
        ...
        if sector == None or sector == "global":
            ...
            if dlim is None and dStd > 0:
                cmap, norm, dextend = chooseColorLevels(
                    dMean - sigma * dStd,
                    dMean + sigma * dStd,
                    dcolormap,
                    clim=dlim,
...
def yzplot(
  ...
  """
    ...
    # Create coordinates if not provided
    ylabel, yunits, zlabel, zunits = createYZlabels(
    ...
    if len(ylabel + yunits) > 0:
        plt.xlabel(label(ylabel, yunits))
...
def yzcompare(
  ...
  """
    ...
    dMin, dMax, dMean, dStd, dRMS = myStats(
        maskedField1 - maskedField2, yzWeighting, debug=debug
    )
    dRxy = corr(maskedField1 - s1Mean, maskedField2 - s2Mean, yzWeighting)
    s12Min = min(s1Min, s2Min)
    s12Max = max(s1Max, s2Max)
    xLims = numpy.amin(yCoord), numpy.amax(yCoord)
    yLims = boundaryStats(zCoord)
    if debug:
        print("s1: min, max, mean =", s1Min, s1Max, s1Mean)
        print("s2: min, max, mean =", s2Min, s2Max, s2Mean)
        print("s12: min, max =", s12Min, s12Max)
    ...
    if npanels in [2, 3]:
        ...
        if interactive:
            addStatusBar(yCoord, zCoord, field1)
        cb1 = plt.colorbar(fraction=0.08, pad=0.02, extend=extend)
        if centerlabels and len(clim) > 2:
            cb1.set_ticks(0.5 * (clim[:-1] + clim[1:]))
        axis.set_facecolor(landcolor)
    ...
    if npanels in [1, 3]:
        ...
        if interactive:
            addStatusBar(yCoord, zCoord, field1 - field2)
        cb3 = plt.colorbar(fraction=0.08, pad=0.02, extend=dextend)
        if centerdlabels and len(dlim) > 2:
            cb3.set_ticks(0.5 * (dlim[:-1] + dlim[1:]))
        axis.set_facecolor(landcolor)
        if splitscale is not None:
...
def linCI(min, max, ci, *args):
  ...
  """
    if len(args):
        return numpy.concatenate((numpy.arange(min, max + ci, ci), linCI(*args)))
...
def pmCI(min, max, ci, *args):
  ...
  """
    ...
    if ci[0] > 0:
        return numpy.concatenate((-ci[::-1], ci))
    else:
        return numpy.concatenate((-ci[::-1], ci[1:]))
...
def myStats(s, area, s2=None, debug=False):
  ...
  """
    ...
    weight = area.copy()
    if debug:
        print("myStats: sum(area) =", numpy.ma.sum(weight))
    ...
    if debug:
        print("myStats: sum(s) =", numpy.ma.sum(s))
    if debug:
        print("myStats: sum(area*s) =", numpy.ma.sum(weight * s))
  ...
  """
    ...
    def keyPress(event):
        if event.key == "Q":
            ...
            plt.close()  # Close just the active figure
...
def plotBasemapPanel(
    field, sector, xCoord, yCoord, lonRange, latRange, cmap, norm, interactive, extend
):
    if sector == "arctic":
        m = Basemap(projection="npstere", boundinglat=60, lon_0=-120, resolution="l")
    elif sector == "shACC":
        m = Basemap(projection="spstere", boundinglat=-45, lon_0=-120, resolution="l")
    else:
        m = Basemap(
            projection="mill",
            lon_0=-120.0,
    ...
    m.drawcoastlines()
    if interactive:
        addStatusBar(xCoord, yCoord, field)
    if extend is None:
        extend = extend
